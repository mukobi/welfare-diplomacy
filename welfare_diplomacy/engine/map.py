...
class Map:
    ...
    def svg_path(self):
        ...
        for file_name in [self.name + ".svg", str(self.root_map) + ".svg"]:
    ...
    def validate(self, force=0):
        ...
        # Validating powers
        self.powers = [
            power_name for power_name in self.homes if power_name != "UNOWNED"
        ]
        self.powers.sort()
        ...
        for place, abuts in self.loc_abut.items():
            ...
            for coast_loc in self.find_coasts(place):
                ...
                adj_water_locs |= {
                    loc.upper()
                    for loc in self.loc_abut[coast_loc]
                    if self.area_type(loc) == "WATER"
                }
        ...
        for power_name, places in self.centers.items():
            for loc in places:
                if not self.area_type(loc):
                    self.error.append(
                        err.MAP_BAD_INITIAL_OWN_CENTER % (power_name, loc)
                    )
    ...
    def load(self, file_name=None):
        ...
        if file_name is None:
            file_name = (
                "{}.map".format(self.name)
                if not self.name.endswith(".map")
                else self.name
            )
        ...
        with open(file_path, encoding="utf-8") as file:
            ...
            for line in file:
                ...
                # -- [oldAbbrev ->] placeName = abbreviation alias...
                    ...
                    if not (word[0][0] + word[0][-1]).isalnum() or word[0] != self.norm(
                        word[0]
                    ).replace(" ", ""):
                    ...
                    for alias in word[1:]:
                        ...
                        normed = (
                            alias[:-1].replace("+", " ").upper()
                            if unclear
                            else self.norm(alias)
                        )
                        if unclear:
                ...
                # -- CENTERS [center...]
                    ...
                    if not power:
                        self.error += [
                            err.MAP_OWNS_BEFORE_POWER % (upword, " ".join(word))
                        ]
                    else:
                        ...
                        if upword[0] == "C" or power not in self.centers:
                            ...
                            self.centers[power].extend(
                                [
                                    center
                                    for center in line.upper().split()[1:]
                                    if center not in self.centers[power]
                                ]
                            )
                ...
                # -- HOME(S) [center...]
                    ...
                    if not power:
                        self.error += [
                            err.MAP_HOME_BEFORE_POWER % (upword, " ".join(word))
                        ]
                    else:
                    ...
                    # DUMMY powerName powerName
                        ...
                        self.dummies.extend(
                            [
                                dummy
                                for dummy in [
                                    self.norm_power(p_name) for p_name in word[1:]
                                ]
                    ...
                    # DUMMY ALL EXCEPT powerName powerName
                        ...
                        self.dummies = [
                            ...
                            not in (
                                ["UNOWNED"]
                                + [
                                    self.norm_power(except_pow)
                                    for except_pow in word[3:]
                                ]
                            )
                        ]
                ...
                # -- UNPLAYED [ALL EXCEPT] powerName...
                    ...
                    # UNPLAYED powerName powerName
                        ...
                        goners = [
                            self.norm_power(power_name) for power_name in word[1:]
                        ]
                    # UNPLAYED ALL
                    elif len(word) == 2:
                        goners = [
                    ...
                    # UNPLAYED ALL EXCEPT powerName
                        ...
                        goners = [
                            ...
                            not in (
                                ["UNOWNED"]
                                + [
                                    self.norm_power(pow_except)
                                    for pow_except in word[3:]
                                ]
                            )
                        ]
                    ...
                    if power and not self.pow_name.get(power):
                        ...
                        if normed not in self.aliases:
                            if len(normed.split("/")[0]) in (1, 3):
                                self.error += [
                                    err.MAP_POWER_NAME_CAN_BE_CONFUSED % normed
                                ]
    ...
    def build_cache(self):
        ...
        for loc in self.locs:
            self.loc_coasts[loc.upper()] = [
                map_loc.upper()
                for map_loc in self.locs
                if loc.upper()[:3] == map_loc.upper()[:3]
            ]
        ...
        for loc in self.locs:
            ...
            self.dest_with_coasts[loc] = list(
                {val for sublist in dest_with_coasts for val in sublist}
            )
    ...
    def norm(self, phrase):
        ...
        return " ".join(
            [self.keywords.get(keyword, keyword) for keyword in phrase.strip().split()]
        )
    ...
    def vet(self, word, strict=0):
        ...
        for thing in word:
            if " " in thing:
                ...
                data_type = POWER
            if strict and thing not in list(self.aliases.values()) + list(
    ...
    def rearrange(self, word):
        ...
        while ("~", OTHER) in result:
            ...
            del result[i]
            if (
                result[i][1] not in (LOCATION, COAST)
        ...
        for i in range(len(result) - 1, 1, -1):
            if result[i][1] in (LOCATION, COAST) and result[i - 1][1] in (
                LOCATION,
                COAST,
            ):
                result[i:i] = [("-", MOVE_SEP)]
    ...
    def default_coast(self, word):
        ...
        if len(word) == 4 and word[0] == "F" and word[2] == "-" and "/" not in word[3]:
            ...
            for place in self.abut_list(unit_loc):
                ...
                if (
                    new_loc == up_place
                ):  # Target location found with no coast, original query is correct
                    return word
                if new_loc == up_place[:3]:
                    if (
                        single_coast
                    ):  # Target location has multiple coasts, unable to decide
                        return word
                    single_coast = up_place  # Found a potential candidate, storing it
            word[3] = (
                single_coast or new_loc
            )  # Only one candidate found, modifying the order to include it
        return word
    ...
    def abuts(self, unit_type, unit_loc, order_type, other_loc):
        ...
        **Note**: This method uses the precomputed cache
        ...
        """
        if unit_type == "?":
            return self.abuts_cache.get(
                ("A", unit_loc.upper(), order_type, other_loc.upper()), 0
            ) or self.abuts_cache.get(
                ("F", unit_loc.upper(), order_type, other_loc.upper()), 0
            )
    ...
    def _abuts(self, unit_type, unit_loc, order_type, other_loc):
        ...
        **Note**: This method is used to generate the abuts_cache
        ...
        elif order_type != "C" and (
            other_loc_type == "WATER" or place == place.title()
        ):
            return 0
    ...
    def compare_phases(self, phase1, phase2):
        ...
        if season_ix1 > season_ix2:
            return (
                -1
                if "NEWYEAR"
                in [x.split()[0] for x in self.seq[(season_ix2) + (1) : season_ix1]]
                else 1
            )
        if season_ix1 < season_ix2:
            return (
                1
                if "NEWYEAR"
                in [x.split()[0] for x in self.seq[(season_ix1) + (1) : season_ix2]]
                else -1
            )
        return 0
    ...
    def phase_abbr(phase, default="?????"):
        ...
        return (
            ("%.1s%04d%.1s" % (parts[0], int(parts[1]), parts[2])).upper()
            if len(parts) == 3
            else default
        )
    ...
    def phase_long(self, phase_abbr, default="?????"):
        ...
        try:
            ...
            for season in self.seq:
                parts = season.split()
                if (
                    parts[0] not in ("NEWYEAR", "IFYEARDIV")
...
from welfare_diplomacy.utils.convoy_paths import (
    add_to_cache,
    get_convoy_paths_cache,
