...
def main():
    ...
    parser.add_argument(
        "--input", "-i", type=str, required=True, help="SVG file to convert."
    )
...
export class %(classname)s extends React.Component {
    ...
    render() {
        ...
        if (game.isWelfare()) {
            nb_centers_per_power = "S/U/W: " + nb_centers.map((power_data) => (power_data[0] + ' ' + power_data[1] + '/' + power_data[2] + '/' + power_data[3])).join(' ');
        }
        else {
